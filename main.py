...
def wait_for_valid_token(controller):
    """
    ...
    while True:
        ...
        if token:
            if can_connect_with_token(token):
                ...
                TwitchAuthHandler().refresh_twitch_token()
        else:
...
def start_bot_loop(controller, shared_queue_manager):
    ...
    wait_for_valid_token(controller)
...
if __name__ == "__main__":
    ...
    bot_loop_thread = threading.Thread(
        target=start_bot_loop, args=(controller, shared_queue_manager), daemon=True
    )
